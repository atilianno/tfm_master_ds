...
```{r}
#uno
fviz_nbclust(x = df_soja_normalizado, FUNcluster = kmeans, method = "wss", k.max = 15, 
             diss = get_dist(df_soja_normalizado, method = "euclidean"), nstart = 50)
#dos
d2f=data.frame(df_soja_normalizado)
...
data(iris)
# PCA
pca <- prcomp(df_soja_normalizado)
df_soja_normalizado.pca <- pca$x
# Cluster over the three first PCA dimensions
kc <- kmeans(df_soja_normalizado.pca[,1:3], 3)
