...
```{r}
seitan_b <- seitan_b %>% 
  mutate(outlier = ifelse(lof_score > 1.8334693, 1, 0))
...
```{r}
df_seitan <- df_seitan[seitan_lof < 1.8334693,]
df_seitan_outliers <- df_seitan[seitan_lof >= 1.8334693,]
```
...
```{r}
quantile(soja_b$lof_score, probs = c(0, 0.9))

```

```{r}
soja_b <- soja_b %>% 
  mutate(outlier = ifelse(lof_score > 1.7592718, 1, 0))
...
```{r}
df_soja <- df_soja[soja_lof < 1.7592718,]
df_soja_outliers <- df_soja[soja_lof >= 1.7592718,]
...
set.seed(123)
km_clusters <- kmeans(x = s_soja, centers = 3, nstart = 50)
